...
def convert_object(image, screen_size = None, isDebug = False):
    ...
    if isDebug  : pass#cv2.imshow('edged', edged)
    ...
    if isDebug : pass #cv2.imshow('drawn countours', cv2.drawContours(imageCopy, countours, -1, (0, 255, 0), 1))
    ...
    if isDebug :   pass #cv2.imshow(" Screen", cv2.drawContours(image.copy(), [screenCntList[0]], -1, (0, 255, 0), 3))
    ...
    if(isDebug):
        ...
        cv2.imwrite("2_Cropprd_Image.jpg",warped)
        cv2.waitKey(0)
...
convert_object(cv2.imread('1_Whole_Surgical_Tray.jpg'), isDebug=True)